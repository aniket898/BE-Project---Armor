<?php 
    ...
    #Ensure that the client has provided a value for "searchnameToSearch" 
	...
	{ 
        ...
        $query="SELECT * FROM categorydb WHERE name = '$search_name'";
		...
		$cid=$row[0]; 	
		#get the mapping binary pattern
		#Escape special characters to avoid SQL injection attacks 
        $search_name = mysqli_real_escape_string($con, $search_name); 
        $query="SELECT * FROM mapdb WHERE id = $cid";
        #Query the database to get the user details. 
        $userdetails = mysqli_query($con,$query ); 
        #If no data was returned, check for any SQL errors 
        if (!$userdetails) 
        ...
        #Get the first row of the results 
        $row = mysqli_fetch_row($userdetails); 
		#print "got result as " . $row[0] . " and " . $row[1]; 
        #Build the result array (Assign keys to the values)
		$bitmap =$row[1];
        $result_data = array( 
			'id' => $cid,
